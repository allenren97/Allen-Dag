...
This template is engine-agnostic. It imports ``IMPORT_CONNECTORS`` (a
``{engine: connector_class}`` registry built by
``common_lib/connector_class/__init__.py``) and dispatches by the YAML's
``engine`` field. Adding support for a new source database is purely
additive — drop a new ``common_lib/connector_class/<engine>.py`` file
with a ``BaseImportConnector`` subclass that sets ``ENGINE = "<engine>"``
